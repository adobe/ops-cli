...
def skms(args):
    ...
    for info in response['data']['result_arr'][0]['data']['results']:
        ...
        if info['primary_ip_address'] is None:
            display.display("[WARN] missing primary_ip_address, ignoring following host SKMS data:\n" + str(info), color='yellow')
            continue
        ...
        info['environment'] = info['environment'].replace(environment_strip,'')
        ...
        for device_service in info['device_service']:
            device_service = device_service.replace(device_service_strip,'').encode('utf-8')
        ...
        if info['name'] not in dictionary_of_hosts[info['owner']]['hosts']:
            dictionary_of_hosts[info['owner']]['hosts'].append(info['name'].encode('utf-8'))
        ...
        if info['name'] not in dictionary_of_hosts[info['site']]['hosts']:
            dictionary_of_hosts[info['site']]['hosts'].append(info['name'].encode('utf-8'))

        # add cluster to top-level dictionary as a group
        if info['cluster'] not in dictionary_of_hosts:
            dictionary_of_hosts[info['cluster'].encode('utf-8')] = {"hosts" : []}
        if info['name'] not in dictionary_of_hosts[info['cluster']]['hosts']:
            dictionary_of_hosts[info['cluster']]['hosts'].append(info['name'].encode('utf-8'))
